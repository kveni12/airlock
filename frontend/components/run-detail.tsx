    ...
    <header className="card p-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          <Link href={`/workbench/${run.id}`} className="inline-flex items-center gap-1.5 rounded-lg bg-[#182a33] px-3 py-2 text-xs font-semibold text-white hover:bg-[#182a33]"><PanelsTopLeft className="size-3.5" />Open in workbench</Link>
...
export function AlignmentTile({ label, segment, counts }: { label: string; segment?: AlignmentSegment; counts?: RunDetailModel["alignment"]["counts"] }) {
  ...
  return <div className="rounded-xl border bg-[#f6f2ec] p-4">
    ...
    {counts && <p className="mt-2 text-xs text-[#64717c]">{deviations} undeclared · {counts.missingExpectedActions} missing expected</p>}
    {segment && segment.findingIds.length > 0 && <p className="mt-1 text-xs text-[#64717c]">{segment.findingIds.length} finding{segment.findingIds.length === 1 ? "" : "s"}</p>}
  </div>;
}
...
function ObservedList<T extends ObservedItem>({ items, flag, suffix }: { items: T[]; flag?: (item: T) => boolean; suffix?: (item: T) => string | undefined }) {
  ...
  return <ul className="space-y-1">{items.map((item, index) => {
    ...
    return <li key={`${item.name}-${item.eventIds[0] ?? index}`} className="flex flex-wrap items-center gap-2 text-sm"><span className={undeclared ? "text-[#815017]" : "text-[#14623f]"}>{undeclared ? "⚠" : "✓"}</span><span className="mono">{item.name}</span>{extra && <span className="text-xs text-[#64717c]">{extra}</span>}{undeclared && <span className="status status-warn">undeclared</span>}<VerificationBadge verification={item.verification} /><span className="text-xs text-[#98a4ad]">{item.eventIds.length} event{item.eventIds.length === 1 ? "" : "s"}</span></li>;
  })}</ul>;
}
...
export function ResultSection({ result, detail }: { result: ResultSummary; detail: RunDetailModel }) {
  ...
  return <Section eyebrow="5 · Result" title="What the agent produced" action={<span className={`status ${approval === "approved" ? "status-good" : approval === "needs_human" ? "status-warn" : "status-muted"}`}>{approval.replace("_", " ")}</span>}>
    <dl className="space-y-3">
      ...
      <KeyValue label="Review">{result.review ? <span>{result.review.filesReviewed} / {result.review.filesTotal} files reviewed · {result.review.filesWithFindings} with findings · <Link className="underline" href={`/reviews/${result.review.reviewId}`}>{result.review.status.replace("_", " ")}</Link>{result.review.approvedAt && ` · approved ${formatDateTime(result.review.approvedAt)}${result.review.approval?.actor ? ` by ${result.review.approval.actor}` : ""}`}</span> : <span className="text-sm text-[#64717c]">not reviewed</span>}</KeyValue>
