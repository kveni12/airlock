  ...
  return <div className="mx-auto max-w-6xl space-y-6">
    ...
    <header className="card p-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <div className="flex flex-wrap items-center gap-2"><RunStatusBadge status={run.status} /><span className="status capitalize">{run.purpose ?? "builder"}</span>{run.workspaceAccess === "read_only" && <span className="status status-muted">read-only workspace</span>}<span className="mono text-xs text-[#657068]">{run.runtimeProvider}</span></div>
          <h1 className="mt-3 text-3xl font-semibold tracking-[-.04em]">{run.taskId}</h1>
          <p className="mono mt-1 text-xs text-[#657068]">{run.id} · agent {run.agentId} · {formatDateTime(run.createdAt)}{run.completedAt && ` → ${formatTime(run.completedAt)}`}{run.exitCode != null && ` · exit ${run.exitCode}`}</p>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {d.review && <Link href={`/reviews/${d.review.id}`} className="rounded-lg border bg-white px-3 py-2 text-xs font-semibold hover:bg-[#f4f7f1]">Open review</Link>}
        </div>
      </div>
...
function AlignmentTile({ label, segment, counts }: { label: string; segment?: AlignmentSegment; counts?: RunDetailModel["alignment"]["counts"] }) {
  ...
  return <div className="rounded-xl border bg-[#f7f9f4] p-4">
    ...
    <p className="mt-2 text-sm text-[#3a443c]">{segment?.detail ?? "Not evaluated."}</p>
    {counts && <p className="mt-2 text-xs text-[#657068]">{deviations} undeclared · {counts.missingExpectedActions} missing expected</p>}
    {segment && segment.findingIds.length > 0 && <p className="mt-1 text-xs text-[#657068]">{segment.findingIds.length} finding{segment.findingIds.length === 1 ? "" : "s"}</p>}
  </div>;
}
...
function RequestSection({ request, analysis }: { request?: HumanRequest; analysis?: RequestAnalysis }) {
  return <Section eyebrow="1 · Human request" title={request ? "What the human asked for" : "No human request linked"}>
    {!request ? <p className="text-sm text-[#657068]">This run was started without a first-class request. Alignment for Request → Intent cannot be evaluated.</p> : <div className="space-y-4">
      ...
      <p className="mono text-xs text-[#657068]">{request.id} · {formatDateTime(request.createdAt)} · raw prompt is immutable</p>
      {analysis && <dl className="grid gap-3 md:grid-cols-2">
        ...
        <div><dt className="text-xs font-semibold uppercase tracking-wider text-[#657068]">Forbidden resources</dt><dd className="mt-1.5">{analysis.explicitlyForbiddenResources.length ? <ul className="space-y-1 text-sm">{analysis.explicitlyForbiddenResources.map((r) => <li key={r.resource} className="flex flex-wrap items-center gap-2"><span className="mono">{r.resource}</span><span className="status status-muted">{r.category}</span><span className="text-xs text-[#657068]">“{r.excerpt}”</span></li>)}</ul> : <span className="text-sm text-[#657068]">none</span>}</dd></div>
        {analysis.ambiguities.length > 0 && <div className="md:col-span-2"><dt className="text-xs font-semibold uppercase tracking-wider text-[#657068]">Ambiguities</dt><dd className="mt-1.5 text-sm">{analysis.ambiguities.join("; ")}</dd></div>}
      </dl>}
    </div>}
  </Section>;
}
...
function BehaviorSection({ behavior, intent }: { behavior?: ObservedBehavior; intent?: AgentIntent }) {
  ...
  return <Section eyebrow="4 · Observed behavior" title="What Periscope observed">
    <dl className="space-y-3">
      ...
      <KeyValue label="Files deleted"><ObservedList items={behavior.files.deleted} flag={(i) => intent ? !matches(files, i.name) : false} /></KeyValue>
      <KeyValue label="Files read"><UnavailableNote reason={behavior.files.read.reason} /></KeyValue>
      <KeyValue label="Commands"><ObservedList items={behavior.commands} suffix={(i) => i.exitCode != null ? `exit ${i.exitCode}` : undefined} /></KeyValue>
      <KeyValue label="Tests"><ObservedList items={behavior.tests} suffix={(i) => i.passed === undefined ? "result unknown" : i.passed ? "passed" : "failed"} /></KeyValue>
      <KeyValue label="Dependencies added"><ObservedList items={behavior.dependenciesAdded} flag={(i) => intent ? !deps.has(i.name) : false} /></KeyValue>
      <KeyValue label="Network destinations"><ObservedList items={behavior.networkDestinations} flag={(i) => intent ? !net.has(i.name) : false} suffix={(i) => i.allowed === false ? "blocked by policy" : undefined} /></KeyValue>
    ...
    <div className="mt-5 border-t pt-4">
      <p className="text-xs font-semibold uppercase tracking-wider text-[#657068]">Telemetry coverage</p>
      <p className="mt-1 text-xs text-[#657068]">Channels marked unavailable cannot be observed; their absence above is not evidence that nothing happened.</p>
...
function ResultSection({ result, detail }: { result: ResultSummary; detail: RunDetailModel }) {
  ...
  return <Section eyebrow="5 · Result" title="What the agent produced" action={<span className={`status ${approval === "approved" ? "status-good" : approval === "needs_human" ? "status-warn" : "status-muted"}`}>{approval.replace("_", " ")}</span>}>
    <dl className="space-y-3">
      ...
      {detail.resolutions.length > 0 && <KeyValue label="Resolutions"><ul className="space-y-1 text-sm">{detail.resolutions.map((r) => <li key={r.id} className="flex flex-wrap items-center gap-2"><span className={`status ${r.status === "resolved" ? "status-good" : r.status === "failed" ? "status-bad" : "status-info"}`}>{r.status.replace("_", " ")}</span><span className="mono text-xs">{r.findingId}</span>{r.resolutionRunId && <Link className="text-xs underline" href={`/runs/${r.resolutionRunId}`}>resolver run</Link>}{r.reReviewResult && <span className="text-xs text-[#657068]">{r.reReviewResult.summary}</span>}</li>)}</ul></KeyValue>}
    </dl>
  </Section>;
}
...
function Timeline({ entries, error }: { entries: TimelineEntry[]; error: string | null }) {
  ...
  return <ol className="card divide-y">{entries.map((entry) => <li key={entry.id} className="grid gap-2 p-4 sm:grid-cols-[88px_110px_minmax(0,1fr)] sm:items-start">
    <span className="mono text-xs text-[#657068]">{formatTime(entry.timestamp)}</span>
    <span className={`status justify-self-start border-transparent uppercase ${actorStyle[entry.actor]}`}>{entry.actor}</span>
    <div className="min-w-0">
      <div className="flex flex-wrap items-center gap-2"><span className="text-sm font-semibold">{entry.title}</span><span className="mono text-[10px] text-[#9ca99d]">{entry.kind}</span>{entry.severity && entry.severity !== "info" && <SeverityBadge severity={entry.severity} />}<VerificationBadge verification={entry.verification} />{entry.evidenceSource && <span className="text-[10px] text-[#9ca99d]">via {entry.evidenceSource}</span>}</div>
      {entry.detail && <p className="mt-1 whitespace-pre-wrap break-words text-sm text-[#3a443c]">{entry.detail}</p>}
      <p className="mono mt-1 text-[10px] text-[#9ca99d]">{Object.entries(entry.refs).filter(([, v]) => v && (!Array.isArray(v) || v.length)).map(([k, v]) => `${k}=${Array.isArray(v) ? v.join(",") : v}`).join(" · ")}</p>
    </div>
  </li>)}</ol>;
}
