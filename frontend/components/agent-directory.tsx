...
export function AgentDirectory() {
  ...
  return <Page>
    ...
    <div className="grid gap-4 lg:grid-cols-2">{runs.map((run) => {
      ...
      return <div key={run.agentId} className="card p-6"><div className="flex items-start justify-between gap-4"><div className="flex gap-3"><span className="grid size-10 place-items-center rounded-lg bg-[#182a33] text-[#ecdcc8]"><Bot className="size-5" /></span><div><h2 className="font-semibold">{displayAgent(run)}</h2><p className="mt-1 text-sm text-[#64717c]">{run.agent?.kind ?? "generic"} · {run.agent?.prompt ?? run.taskId}</p></div></div><span className={`status ${highRisk ? "status-warn" : "status-good"}`}>{highRisk ? "High risk" : statusLabel(run.status)}</span></div>
