...
function riskReason(runs: RunRecord[], eventsByRun: Record<string, AgentEvent[]>): string | null {
  ...
  const kinds = new Set(perRun.flat().map((event) => /write|filesystem|path/.test(event.action) ? "wrote outside scope" : /network|host|egress/.test(event.action) ? "reached a blocked host" : /secret/.test(event.action) ? "touched an ungranted key" : "broke policy"));
            ...
            {runs.map((run) => {
              ...
              return <tr key={run.agentId} className="align-top transition-colors hover:bg-[#f7f8f8]">
                ...
                <td className="w-32 px-4 py-4">
                  {reason ? <span className="status status-warn" title={reason}>High risk · {reason}</span> : <RunStatusBadge status={run.status} />}
