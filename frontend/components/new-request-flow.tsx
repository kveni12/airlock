  ...
  return <div className="mx-auto max-w-4xl space-y-3">
    ...
    <Section eyebrow="Step 1" title="Human request" action={requestResult ? <span className="status status-good"><Check className="mr-1 size-3.5" />recorded</span> : <Link href="/requests/rules" className="inline-flex items-center gap-1.5 text-xs font-semibold text-[#64717c] hover:text-[#182a33]"><Settings2 className="size-3.5" />Edit extraction rules</Link>}>
      <div className="space-y-3">
        ...
        <label className="block text-sm"><span className="text-xs font-semibold uppercase tracking-wider text-[#64717c]">Prompt (stored verbatim, immutable)</span><textarea value={prompt} disabled={Boolean(requestResult)} onChange={(e) => { setPrompt(e.target.value); setPreview(null); }} rows={4} placeholder="e.g. Fix the login bug and add a regression test. Do not modify the database." className="mt-1 w-full rounded-lg border bg-white px-3 py-2 text-sm placeholder:italic placeholder:text-[#9aa5ad] disabled:bg-[#f0f2f3]" /></label>
        {!requestResult && !preview && <div className="flex flex-wrap items-center gap-3"><ActionButton disabled={!prompt.trim() || !taskId.trim()} onClick={async () => { try { await analyzePrompt(); } catch (e) { setError(e instanceof Error ? e.message : String(e)); throw e; } }}>Analyze prompt</ActionButton><button type="button" onClick={() => { setPrompt(DEMO_PROMPT); setPreview(null); }} className="text-xs font-semibold text-[#64717c] underline-offset-2 hover:text-[#182a33] hover:underline">Use the demo prompt</button></div>}
