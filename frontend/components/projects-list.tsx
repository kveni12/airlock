...
import { summarizeScope, type AccessScope } from "./access-scope";
...
export function ProjectsList() {
  ...
  const open = async (project: Project) => {
    ...
    router.push(`/requests/new?project=${encodeURIComponent(project.id)}`);
  };

  const items = [...(projects.data ?? [])].sort((a, b) => (b.lastOpenedAt ?? b.updatedAt).localeCompare(a.lastOpenedAt ?? a.updatedAt));

  return <div className="space-y-4">
    <Section eyebrow="Projects" title="Repos and their saved access settings" action={!creating && <ActionButton onClick={() => setCreating(true)}><Plus className="mr-1 inline size-4" />New project</ActionButton>}>
      <p className="text-sm text-[#64717c]">A project remembers where the repo lives and what an agent may see and change there — folders (read only vs can change), internet hosts, secrets, MCP servers, tools, runtime and default agent. Open one and New request starts from those settings.</p>
      {creating && <div className="mt-4 rounded-xl border bg-[#f6f2ec] p-4">
        <ProjectForm initial={EMPTY_PROJECT} submitLabel="Create project" onCancel={() => setCreating(false)} onSubmit={async (input) => { await createProject(input); setCreating(false); await projects.refresh(); }} />
      </div>}
    </Section>
    {projects.error && <ErrorBanner message={projects.error} onRetry={projects.refresh} />}
    {!projects.loading && !items.length && !creating && <Empty title="No projects yet" body="Create one to save a repo's access settings and reuse them for every request." action={<ActionButton onClick={() => setCreating(true)}>New project</ActionButton>} />}
    <ul className="grid gap-3 md:grid-cols-2">{items.map((project) => {
      const s = summarizeScope(project.scope as AccessScope, project.runtime);
      return <li key={project.id} className="card flex flex-col gap-3 p-4">
        ...
        <div className="mt-auto flex flex-wrap items-center gap-2 pt-1">
          <ActionButton onClick={() => open(project)}>Open → New request</ActionButton>
