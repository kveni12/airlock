...
export function OverviewDashboard() {
  ...
  const latestRun = snapshot?.runs.slice().sort((a, b) => b.createdAt.localeCompare(a.createdAt))[0];
    ...
    {snapshot && snapshot.runs.length === 0 ? <EmptyState title="No agent runs yet" body="Create a request and start a run from the New request page, or seed data with npm run demo:intent." /> : <>
      ...
      <section className="mt-5 grid gap-4 lg:grid-cols-[1.25fr_.75fr]">
        <div className="card p-6">
          <div className="flex items-start justify-between gap-4"><div><p className="text-sm text-[#64717c]">Latest run</p><h2 className="mt-1 text-xl font-semibold">{latestRun?.agent?.prompt || latestRun?.taskId || "Add OAuth support"}</h2><p className="mono mt-1 text-xs text-[#64717c]">{latestRun?.repoBranch || "agent/oauth-support"}</p></div><span className={`status ${latestRun?.status === "failed" ? "status-warn" : "status-info"}`}>{latestRun ? statusLabel(latestRun.status) : "Demo"}</span></div>
        ...
        <div className={`card p-6 ${latestPolicyEvent || !snapshot ? "border-amber-300 bg-amber-50/50" : ""}`}><div className="flex gap-3"><Activity className="mt-0.5 size-5 text-[#9a5a13]" /><div><p className="text-sm text-[#64717c]">Latest policy signal</p><h2 className="mt-2 font-semibold">{latestPolicyEvent?.action ? statusLabel(latestPolicyEvent.action) : snapshot ? "No policy violations" : "Unexpected secret access"}</h2><p className="mt-2 text-sm leading-6 text-[#64717c]">{latestPolicyEvent?.resource || (snapshot ? "All recorded activity is within declared policy." : "Demo evidence: GOOGLE_CLIENT_SECRET was accessed outside declared intent.")}</p></div></div></div>
      </section>
    </>}
  </Page>;
}
...
function Metric({ icon: Icon, label, value, detail, warn = false }: { icon: typeof Bot; label: string; value: string | number; detail: string; warn?: boolean }) { return <div className={`card p-5 ${warn ? "border-amber-300" : ""}`}><div className="flex justify-between"><div><p className="text-sm text-[#64717c]">{label}</p><p className="mt-2 text-3xl font-semibold">{value}</p></div><span className="grid size-9 place-items-center rounded-lg bg-[#e6e9eb]"><Icon className="size-4" /></span></div><p className="mt-4 text-xs text-[#64717c]">{detail}</p></div>; }
